...
Authentication, multi-tenancy, RBAC, company onboarding/approval, social-account bookkeeping, and AI creative generation (ad copy, images, and carousels via Gemini; story reels via Gemini Veo/Omni; avatar reels via HeyGen's talking-avatar API) for the aiDigiPlanner social media automation SaaS. Publishing/scheduling and billing are out of scope for this codebase — see [Out of scope](#out-of-scope) below.
...
Creative generation additionally uses: `arq` + Redis (job queue), `google-genai` (Gemini text/image/video), `httpx` (HeyGen REST calls), `Pillow` (image compositing), `ffmpeg-python` + a real `ffmpeg` binary on `PATH` (reel assembly), `Jinja2` (prompt templates), `tenacity` (provider retries).
...
6. **Creative generation prerequisites** (skip this if you're not touching `/api/v1/projects/{id}/creatives/*`):
   ...
   - **`HEYGEN_API_KEY`** + **`HEYGEN_DEFAULT_VOICE_ID`** (optional) — same fallback-to-mock behavior as `GEMINI_API_KEY` above, but gates only `reel_style: "avatar"` reels (see below), which render through HeyGen's talking-avatar API instead of Gemini Veo/Omni. Get a key at [app.heygen.com/settings?nav=API](https://app.heygen.com/settings?nav=API) and a `voice_id` from HeyGen's voice list; every avatar reel currently speaks in this one configured voice. A brand's HeyGen avatar (registered from its uploaded avatar photo, costs real money to create) is persisted to `BrandProfile.heygen_avatar_id` the first time it's generated and reused on every job after — `HEYGEN_DEFAULT_AVATAR_ID` is an optional fallback for a brand that hasn't had one registered yet.
...
2. **Generate** — `POST /api/v1/projects/{project_id}/creatives/generate` (`project_admin`/`editor`; this spends money once a real API key is configured). Body: `product_line` (validated against the brand profile's product lines), `topic`, `format` (`post`/`carousel`/`reel`), `concept_count`, `quality` (`draft`/`standard`/`hero`), and format-specific fields (`carousel_slides`; `reel_duration_s`, `voiceover`, `reel_style` for reels). Returns `202` with a `GenerationJob` immediately — generation runs asynchronously on the `arq` queue (see prerequisites above).
   ...
   - **Reel styles** (`reel_style`, only for `format: "reel"`): `"story"` (default) — scene-by-scene Gemini Veo/Omni-generated b-roll, each concept's own cover image used as the video's visual reference; `voiceover` defaults to `"silent_text"`. `"avatar"` — every scene instead renders as genuine lip-synced speech from the project's uploaded brand-profile avatar image via HeyGen (not a Veo/Omni reference image), so the same face appears throughout and actually speaks the scene's `vo_line`; requires an avatar to already be uploaded (400 otherwise), and `voiceover` defaults to (and must be) `"native_audio"` -- a talking avatar with `silent_text` is rejected at the brief level, since there'd be nothing to lip-sync.
...
**Provider architecture**: `app/modules/creatives/providers/` — ABCs (`LLMProvider`/`ImageProvider`/`VideoProvider`) with a `Gemini*` implementation and a deterministic `Mock*` implementation each, selected by `providers/factory.py` based on `dry_run` and whether `GEMINI_API_KEY`/`HEYGEN_API_KEY` is configured (same pattern as `EmailService`). Three video backends: Veo (native-audio story reels), Omni Flash (silent/on-screen-text story reels, cheaper), and HeyGen (`heygen_video.py`, all avatar-style reels — a real talking-avatar API, not b-roll) — `domain.video_backend_for_reel()` picks based on `voiceover` and `reel_style`.
